...
---
В классе `Step` задается выполнение команды, а в классе `Player` реализовано перемещение игрока
...
В реализации без паттерна все действия представлены объектом `Step`, в котором хранятся данные о перемещении (`dx`,`dy`), а все передвижение, по сути, лежит на методе `player.move(dx, dy)`
```py
class Step:
...
```
___  
В реализации с паттерном список `steps` содержит объекты-команды, поэтому выполнение шага происходит через вызов `execute()` без дополнительных параметров. В реализации без паттерна в списке steps хранятся только смещения dx и dy, поэтому при выполнении шага необходимо явно передавать объект Player.
